...
def CalcComplex(detector):
    # Environment
    depthComplex = utils.LerpArray(detector.constants['depthComplexityX'], detector.constants['depthComplexityY'],
    ...
    if detector.parameters['temperature'] > constants.nitrogenTemp:
        coolingComplex = 1 - (detector.parameters['temperature'] - constants.nitrogenTemp)/(detector.constants['initAmbientTemp'] +
                detector.constants['tempIncPerKm'] * detector.parameters['depth'] / 1000 - constants.nitrogenTemp)
    else:
        coolingComplex = 5
    ...
    return (depthComplex + vacuumComplex + coolingComplex
                    + stagesComplex + massComplex
                    + powerComplex + materialComplex + roughnessComplex)

def CalcCost(detector):
    # Environment
    depthCosts = np.power(max((detector.parameters['depth'] - 20), 0), 1/3) * 75E5
    vacuumCosts = detector.parameters['pumps'] * detector.constants['vacuumPumpCost']
    if detector.parameters['temperature'] > constants.nitrogenTemp:
        K0 = 0
        K1 = 20102
        K2 = detector.constants['initAmbientTemp'] + detector.constants['tempIncPerKm'] * detector.parameters['depth'] / 1000
    ...
    # Vibration
    vibrationCosts = (
            detector.parameters['sus_length']**2.1
            * detector.parameters['sus_stages']**5.5
            * detector.parameters['mirror_mass']**1.2
        ) * 60 - 60

    # Optics
    powerCosts = 47000 + 25e6 * (detector.parameters['power'] / detector.limits['power'][1])**2
    matCosts = detector.parameters['material'].GetCost(detector)
    roughnessCosts = (
            detector.parameters['mirror_mass']**(2/3)
            * (GetRoughnessLoss(detector.parameters['roughness'])**3
                - GetRoughnessLoss(500)**3)
            / 25 * 8e7
        )
...
class ScoreCalculator:
    ...
    # Getter for frequency range
    # IRS TODO: is this used?
    def GetFreqRange():
    ...
    def GetDetectorDistance(self, m1, m2):
        ...
        # At this pothe GW signal shuts off (for BNS) or transitions into
        ...
        tmp = 5 * np.power(m_chirp, 5/3) * 16 / (96 * np.power(np.pi, 4/3) * np.power(snr_threshold, 2))
        ...
        # is the ASD or PSD. My code reads an PSD (~1e-20) and requires the
        # ASD (~1e-40), so I added the square. And I'm almost certain that
        # I didn't mix up PSD and ASD again. The current code is checked
        # against the observing scenarios paper though.
    ...
    def SensitivityLine(self, f):
        ...
        return sum([model.ComputePoint(f, self.detector)**2 for model in usedNoises])
    ...
    def GetNoiseCurves(self):
        ...
        for f in f_range[0:self.nData]:
            ...
            f_out.append(f)
            for key in curves:
                curves[key].append(self.noiseModels[key].ComputePoint(f, self.detector))
    ...
    def Supernovae(self):
        ...
        excess = max(0, Is - In)
        #Debug.Log("Supernova: " + np.power(excess/4e-20, 3) * 25)
        return (round(np.power(excess/4E-20, 3) * 25))
    ...
    def CalcScore(self):
        ...
        # 3) self made SN number, this should be teaked so that a good mid/high freq detector can
        # achieve *one* detection
        ...
        # Missed sources
        overComplex = max(0, CalcComplex(self.detector) - self.detector.parameters['site'].complexCredits)
        complexScale = 1 - overComplex / self.detector.parameters['site'].complexCredits
        score.supernovaeMissed = int(max(0, np.floor(score.supernovae * (1 -
            complexScale))))
        score.nsnsMissed = int(max(0, np.floor(score.nsns * (1 -
            complexScale))))
        score.bhbhMissed= int(max(0, np.floor(score.bhbh * (1 -
            complexScale))))
        return score